...
**NextPass** is a self-hosted, zero-knowledge password manager with automatic WebAuthn Passkey interception, one-click autofill, Google Account Sign-In, TOTP 2FA codes, payment cards, identities, API secrets, secure notes, and encrypted vault backup/import. Built from scratch - **not** a Vaultwarden fork.
...
| `extension/` | Manifest V3 (Chrome / Edge / Opera GX) | **Built (`v0.14.4`)** - React 18, Vite 5, Tailwind CSS, NextPass Google Auth Onboarding Flow, Shadow DOM Autofill Engine, WebAuthn Proxy |
| `desktop/` | Electron | Phase 5 (Planned NextPass Desktop) |
| `android/` | Kotlin (Native) | Phase 6 (Planned NextPass Mobile) |

## NextPass Features (v0.14.4)
...
## NextPass Zero-Knowledge Security Model

The NextPass server only ever stores ciphertext and wrapped keys. No unencrypted credential material ever leaves the client.
...
# Start NextPass backend server locally (default: http://127.0.0.1:8787)
npm run dev:server

# Build NextPass extension (dist/)
npm run build:ext
```

### Server Configuration
NextPass server configuration is controlled via environment variables (see `server/src/config.ts`):
